...
do
  ...
  -- @param wait how much time to wait when trying to acquire the semaphore.
  ...
  -- Notice that this function might return true *even when it fails to acquire
  -- the semaphore or invoke the async callback* - it will try to rebuild
  -- synchronously when a wait is specified and the semaphore isn't acquired
  -- or the timer-based call fails.
  -- @returns error message as a second return value in case of failure
  rebuild = function(name, callback, version, semaphore, wait)
    ...
    if not ok then
      if wait and wait > 0 then
        log(NOTICE, "could not acquire semaphore, attempting sync rebuild")
    ...
    if wait and wait > 0 then
      ...
      if not ok then
        if wait and wait == 0 then
          log(NOTICE, "could not rebuild asynchronously:",
                       err,
                       ", attempting synchronous rebuild")
          local ok, err = safe_rebuild(callback, current_version)
          release_semaphore(semaphore)
          return ok, err
        end
...
do
  ...
  rebuild_plugins_iterator = function(wait)
    ...
    return rebuild("plugins", update_plugins_iterator, version, plugins_iterator_semaphore, wait)
  end
...
do
  ...
  rebuild_router = function(wait)
    return rebuild("router", update_router, router_version, router_semaphore, wait)
  end
...
return {
  ...
  init_worker = {
    before = function()
      ...
      timer_every(1, function(premature)
        ...
        if not ok then
          log(NOTICE, "failure while rebuilding router via timer: ", err)
        end
        ok, err = rebuild_plugins_iterator()
        if not ok then
          log(NOTICE, "failure while rebuilding plugins_iterator via timer: ", err)
        end
      end)
    end
  },
  ...
  preread = {
    before = function(ctx)
      local ok, err = rebuild_router(get_rebuild_timeout())
      if not ok then
